...
- **CA-001 Code Quality**: Specify architectural boundaries and code quality
  constraints this feature MUST preserve.
- **CA-002 Testing**: Specify required automated test levels and regression-test
  expectations.
- **CA-003 UX Consistency**: Specify cross-adapter behavior expectations and any
  approved deviations.
- **CA-004 Performance**: Specify measurable performance budgets and validation
  method for impacted workflows.
...
- **SC-004**: [Business metric, e.g., "Reduce support tickets related to [X] by
  50%"]
- **SC-005**: [Consistency metric across adapters, e.g., "Equivalent operations
  return equivalent result states and validation outcomes in UI/API/MCP"]
- **SC-006**: [Performance metric tied to budget, e.g., "p95 SetPixel latency
  remains under 20 ms for defined project size"]
